...
export default function StoreFavoriteToggle({ userId, storeId, initialFavorited, onChange }: Props) {
  ...
  async function toggle() {
    ...
    if (next) {
      await supabase.from('store_favorites').insert({ user_id: userId, store_location_id: storeId })
    } else {
      await supabase
  ...
  return (
    <button
      ...
      aria-label={favorited ? 'Remove store from favorites' : 'Add store to favorites'}
